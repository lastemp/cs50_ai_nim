...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        """
        key = (tuple(state), action)  # Convert state to tuple for key
        return self.q.get(key, 0)  # Return 0 if key not found
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        """
        new_value = reward + self.alpha * future_rewards
        self.q[(tuple(state), action)] = old_q + \
            self.alpha * (new_value - old_q)
